...
# Grafana Design Document
https://drive.google.com/file/d/1DTAFS2yDLzumjFHjWddMtxld45Us04eG/view?usp=sharing

## TL;DR

```console
$ helm install grafana -f deploy/values-override.yaml . --namespace monitoring
```

## Security Scan – Skipped Checks:
...
## Introduction
This Helm chart is based on the Open Source chart by Bitnami. It's carefully engineered, actively maintained and are the quickest and easiest way to deploy containers on a Kubernetes cluster that are ready to handle production workloads.
...
To uninstall/delete the `grafana` helm release:
...
```console
$ helm install grafana .  --set livenessProbe.successThreshold=5 -f deploy/values-override.yaml . --namespace monitoring
```
...
```bash
$ helm install grafana -f deploy/values-override.yaml .
```
...
```bash
$ helm upgrade grafana . -f deploy/values-override.yaml --namespace monitoring
```
